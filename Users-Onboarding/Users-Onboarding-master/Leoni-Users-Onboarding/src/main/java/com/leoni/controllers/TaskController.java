...
public class TaskController {
    ...
    public ResponseEntity<TaskDTO> createTask(@RequestBody TaskDTO taskDTO) {
        // Vérifier si le designId existe déjà dans la base de données
        if (taskRepository.existsByDesignId(taskDTO.getDesignId())) {
            return ResponseEntity.status(HttpStatus.CONFLICT).body(null); // Retourner une réponse BadRequest si le designId existe déjà
        }

        // Sinon, créer la nouvelle tâche
